...
PyObject *GetpMatrix(structlpsolvecaller *lpsolvecaller, int element)
{
        ...
        if((element >= sizeof(ar) / sizeof(*ar)) ||
           (ar[element] == NULL)) {
                PyErr_Clear();
                 ErrMsgTxt(lpsolvecaller, "GetpMatrix cannot read string."); // Added by me 
                return(NULL);
        }
        return(ar[element]);
}
...
int GetString(structlpsolvecaller *lpsolvecaller, pMatrix ppm, int element, char *buf, int size, int ShowError)
{
        ...
        item = GetpMatrix(lpsolvecaller, element);
        if((item == NULL) || (PyBytes_AsStringAndSize(item, &ptr, &size1) != 0) || (ptr == NULL)) {
                PyErr_Clear();
                if(ShowError){
                        if (item == NULL){
                                ErrMsgTxt(lpsolvecaller, "Expecting a character element1n.");
                        }else if (ptr == NULL){
                                ErrMsgTxt(lpsolvecaller, "Expecting a character element1p.");
                        }else{
                                ErrMsgTxt(lpsolvecaller, "Expecting a character element1f.");
                        }
           	...
           	// if ((PyString_AsStringAndSize(item, &ptr, &size1) != 0) ||
                if ((PyBytes_AsStringAndSize(item, &ptr, &size1) != 0) ||
                	...
                	PyErr_Clear();
                        ...
                        FreeCellCharItems(pa, i);
                        ErrMsgTxt(lpsolvecaller, "Expecting a character element2.");
                }
